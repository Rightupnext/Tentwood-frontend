...
import{ChevronLeft,ChevronRight,Star,Quote,Users,Globe,Sparkles,TrendingUp,Award}from'lucide-react';
...
const testimonials=[
{name:'Robert Kcarery',role:'XTrip Traveler',image:'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150',rating:5,title:'Excellent Tourist Place!',text:'I had the most amazing trip of my life! Everything, including the guided excursions and the airport pickup, was meticulously organized. The itinerary was well-balanced, and the accommodations were excellent.'},
{name:'Sarah Mitchell',role:'Adventure Seeker',image:'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150',rating:5,title:'Unforgettable Experience!',text:'From start to finish, this was a dream vacation! The attention to detail was incredible, and every moment was perfectly planned. The local guides were knowledgeable and friendly. Highly recommend!'},
{name:'Michael Chen',role:'Family Traveler',image:'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150',rating:5,title:'Perfect Family Vacation!',text:'Traveling with kids can be challenging, but this trip was smooth and enjoyable for everyone. The activities were family-friendly, and the accommodations were spacious and comfortable. Will definitely book again!'}
];
...
@keyframes slideContent{from{opacity:0;transform:translateX(${direction==='right'?'50px':'-50px'})}to{opacity:1;transform:translateX(0)}}
...
<div className="absolute bottom-20 left-1/3 w-64 h-64 bg-[#3CAD9B] rounded-full mix-blend-screen filter blur-3xl animate-float" style={{animationDelay:'3s'}}></div>
<div className="absolute bottom-40 right-1/4 w-96 h-96 bg-[#3CAD9B] rounded-full mix-blend-screen filter blur-3xl animate-float-reverse" style={{animationDelay:'1.5s'}}></div>
</div>
...
{/* Breadcrumb */}
<div className={`max-w-7xl mx-auto mb-6 sm:mb-10 ${loaded?'animate-slideInDown':'opacity-0'}`}>
<div className="flex items-center gap-2 text-sm sm:text-base text-gray-400">
<span className="hover:text-[#3CAD9B] transition-colors duration-300 cursor-pointer transform hover:scale-105 inline-block">Home</span>
...
<div className={`relative h-[400px] sm:h-[500px] lg:h-[600px] ${loaded?'animate-scaleInBounce':'opacity-0'}`}>
{/* Rotating Ring */}
...
{/* Ripple Effect */}
<div className="absolute inset-0 bg-[#3CAD9B] rounded-full opacity-30" style={{animation:'ripple 2s ease-out infinite'}}></div>
<div className="absolute inset-0 bg-[#3CAD9B] rounded-full opacity-30" style={{animation:'ripple 2s ease-out 1s infinite'}}></div>
...
<img src={testimonials[activeTestimonial].image} alt="Main traveler" className="w-full h-full object-cover"/>
...
const positions=['top-8 left-8','top-4 right-12','top-1/3 left-4','bottom-20 right-8','bottom-8 left-16'];
const delays=['0s','0.8s','1.6s','2.4s','3.2s'];
return(
<div key={idx} className={`absolute ${positions[idx]} animate-float z-10`} style={{animationDelay:delays[idx]}}>
<div className="relative group">
...
<img src={avatar} alt={`Traveler ${idx+1}`} className="w-full h-full object-cover"/>
</div>
</div>
</div>
);})}
...
<div className="absolute bottom-4 sm:bottom-8 left-1/2 transform -translate-x-1/2 animate-slideInUp" style={{animationDelay:'0.8s'}}>
...
<div key={idx} className="w-7 h-7 sm:w-9 sm:h-9 rounded-full border-2 border-[#3CAD9B] overflow-hidden">
<img src={avatar} alt="" className="w-full h-full object-cover"/>
</div>
))}
</div>
<div>
<p className="text-base sm:text-lg font-black text-[#3CAD9B]">4k+ Customer</p>
<p className="text-xs sm:text-sm text-gray-400 font-semibold">in Worldwide.</p>
</div>
</div>
</div>
</div>
...
{/* Decorative Icons */}
<div className="absolute top-12 right-4 sm:right-8 animate-float z-10" style={{animationDelay:'1.5s'}}>
...
<div className="absolute bottom-1/3 left-2 sm:left-4 animate-float-reverse z-10" style={{animationDelay:'2.5s'}}>
<div className="bg-gradient-to-br from-[#3CAD9B] to-[#2d9b8a] rounded-2xl p-3 sm:p-4 shadow-2xl transform hover:scale-125 hover:-rotate-12 transition-all duration-300 border border-[#3CAD9B]">
<Sparkles className="w-5 h-5 sm:w-7 sm:h-7 text-white"/>
</div>
</div>

<div className="absolute top-1/3 right-8 sm:right-12 animate-float z-10" style={{animationDelay:'3.5s'}}>
<div className="bg-gradient-to-br from-[#3CAD9B] to-[#2d9b8a] rounded-2xl p-3 sm:p-4 shadow-2xl transform hover:scale-125 hover:rotate-12 transition-all duration-300 border border-[#3CAD9B]">
...
<span className="text-xs sm:text-sm font-bold text-white tracking-wider">TESTIMONIALS</span>
</div>
...
{/* Testimonial Card */}
<div key={activeTestimonial} className="bg-gradient-to-br from-black to-gray-900 backdrop-blur-xl rounded-3xl p-6 sm:p-8 md:p-10 shadow-2xl border-2 border-[#3CAD9B]/30 relative overflow-hidden group hover:shadow-[0_20px_80px_-15px_rgba(60,173,155,0.6)] hover:border-[#3CAD9B] transition-all duration-500 animate-slideContent">
{/* Shimmer Effect */}
<div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
<div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#3CAD9B]/20 to-transparent" style={{animation:'shimmer 2s infinite'}}></div>
</div>
...
<div key={i} className="transform hover:scale-125 hover:rotate-12 transition-all duration-300" style={{animationDelay:`${i*0.1}s`}}>
...
<button key={idx} onClick={()=>{setDirection(idx>activeTestimonial?'right':'left');setActiveTestimonial(idx);}}
className={`h-2 rounded-full transition-all duration-500 transform hover:scale-125 ${idx===activeTestimonial?'w-8 sm:w-12 bg-gradient-to-r from-[#3CAD9B] to-[#2d9b8a] shadow-lg':'w-2 bg-gray-600 hover:bg-[#3CAD9B]'}`}/>
))}
</div>
</div>
</div>
